...
$language_file = array('announcements', 'group', 'survey', 'document', 'ausencia');
...
/*
 * Variaveis para auxilio das datas de ausencia
 */
...
// fim das variaveis de auxilio das datas de ausencia
...
require_once $lib.'ausencia.lib.php';
	...
	if ($submitAnnouncement && empty($emailTitle)) {
		...
		if (isset($id) && $id) {
			...
			if ($ctok == $_POST['sec_token']) {
                ...
                /*
                 * Modifica a variavel post para incluir os observadores da ausencia.
                 */
                 if(Ausencia::is_ocorrencia($_POST['situacao']))
                 {
                     // se post for null todos devem receber notificacao
                     if(is_null($_POST['selectedform']))
                     {
                         $course_code = CourseManager::get_course_code_from_course_id($course_id);
                         $user_list = CourseManager::get_user_list_from_course_code($course_code);
                        foreach ($user_list as $user)
                        {
                            $_POST['selectedform'][] .= "USER:".$user['user_id'];
                        }
                     }

                     $observadores = Ausencia::get_usuarios_observadores($course_id);
                    foreach ($observadores as $observador)
                    {
                        $_POST['selectedform'][] .= $observador;
                    }
                    $result = array_unique($_POST['selectedform']);
                    $_POST['selectedform'] = $result;
                 }
                 /*
                  * Fim da interferencia na variavel POST
                  */
                if (!empty($group_id)) {
                ...
                /* 
                 * módulo de envio de anuncio e email aos acompanhadores de ausencia
                 */
                 if(Ausencia::is_ocorrencia($_POST['situacao']))
                 {
                     $user_id = api_get_user_id();
                     $data_aula = $_POST['fyear'] . "-" . $_POST['fmonth'] . "-" . $_POST['fday'];
                     $data_reposicao = $_POST['end_fyear'] . "-" . $_POST['end_fmonth'] . "-" . $_POST['end_fday'];;
                     $comentario = $_POST['comentarioausencia'];
                     
                    Ausencia::add_ocorrencia($course_id, $insert_id, $_POST['situacao'],$user_id, $data_aula, $data_reposicao, $comentario);
                 }
                 /*
                  * testar se eh um anuncio de ausencia
                  * enviar anuncio de ausencia ao ACV 
                  * enviar anuncio de ausencia via email
                  * finalizacao do modulo de acompanhamento de ausencia
                  */
...
if ($display_form) {
    ...
    if (true) {
// comunicado de ausencia. - MARCO
        /*
         * @TODO: verificar se o teste do action modify resolve
         * Se for uma edição do anúncio não permitir visualizar o comunicado de ausência.
		 * Lembrar de incluir a função javascript no arquivo resources/js/main.js
         */

        echo '<div class="control-group">
		       <div class="controls">
				    <a href="javascript://" onclick="return registrar_ausencia();"><span id="ausencia">&nbsp;<img style="vertical-align:middle;" src="../img/div_show.gif" alt="" />&nbsp;' . get_lang("ComunicadoDeAusencia") . '</span></a>
				    <br />
					<table id="options_ausencia" style="display: none;">
					<tr>
						<td colspan="2"></br>
					        <label for="ocorrencia">' . get_lang('ComunicadoDeAusenciaSituacao') . '&nbsp;</label>
                                                                                                                <select name="situacao" type=ocorrencia>
                                                                                                                   <option value="selecione">' . get_lang('ComunicadoDeAusenciaSelecione') . '</option>
                                                                                                                  <option value="visitatecnica">' . get_lang('ComunicadoDeAusenciaVisitaTecnica') . '</option>
                                                                                                                  <option value="programada">' . get_lang('ComunicadoDeAusenciaProgramada') . '</option>
                                                                                                                  <option value="naoprogramada">' . get_lang('ComunicadoDeAusenciaNaoProgramada') . '</option>
                                                                                                                </select>
        ...
        // start date and time
        echo '<div class="control-group">';
        echo '<label class="control-label">' . get_lang('ComunicadoDeAusenciaDiaNormal') . '</label>
				<div class="controls">
        ...
        echo '<div class="control-group">
				<label class="control-label">
					' . get_lang('ComunicadoDeAusenciaDiaReposicao') . '
				</label>
				<div class="controls">
        ...
        echo '
					 ...
					 <tr>
					    <td colspan="2">
					    	<label for="comentarioausencia">' . get_lang('ComunicadoDeAusenciaComentario') . '</label><br />
					    	<textarea name="comentarioausencia" rows ="4" cols = "64" ></textarea>
					    </td>
                                                                                                    </tr>
