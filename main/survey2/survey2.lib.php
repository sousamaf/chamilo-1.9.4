...
class survey_manager {
	...
	public static function copy_survey($parent_survey, $new_survey_id) {
	    $course_id = api_get_course_int_id();
	 ...
	 * @todo one sql call should do the trick
	 */
	static function get_questions($survey_id, $course_id = '') {
		// Table definitions
		$tbl_survey_question 			= Database :: get_course_table(TABLE_SURVEY_QUESTION);
		$table_survey_question_option 	= Database :: get_course_table(TABLE_SURVEY_QUESTION_OPTION);
	...
	public static function save_question($survey_data, $form_content, $course_id = '') {
		...
		if (strlen($form_content['question']) > 1) { // Checks lenght of the question
			...
			if (!$empty_answer) {
				...
				$message_options=survey_manager::save_question_options($form_content, $survey_data, $course_id);
			} else {
...
class SurveyUtil {
	...
	static function send_invitation_mail2($invitedUser, $invitation_code, $invitation_title, $invitation_text) {
		...
		// Replacing the **link** part with a valid link for the user
		$survey_link = api_get_path(WEB_CODE_PATH).'survey2/fillsurvey.php?course='.$_course['code'].'&invitationcode='.$invitation_code;
		$text_link = '<a href="'.$survey_link.'">'.get_lang('ClickHereToAnswerTheSurvey')."</a><br />\r\n<br />\r\n".get_lang('OrCopyPasteTheFollowingUrl')." <br />\r\n ".$survey_link;
