...
$language_file = array('survey', 'avaliacao_institucional');
...
if (isset($_POST['finish_survey'])) {
	...
	$message = "";
	if($survey_invitation['survey_code'] == ENQUETEALUNO OR $survey_invitation['survey_code'] == ENQUETEPROFESSOR)
	{
		$message = get_lang('AndamentoAutoavaliacao');
	}
	else {
		$dado = AvaliacaoInstitucional::getEnqueteInfo($cpf, $survey_invitation['survey_code']);
		if(AvaliacaoInstitucional::isAllSurveyDone($user_id, $cpf))
			$message = sprintf(get_lang('AndamentoAvaliouTodos'), $dado['disciplina'], $dado['professor']);
		else
			$message = sprintf(get_lang('AndamentoAvaliouUm'), $dado['disciplina'], $dado['professor']);		
	}
	...
	AvaliacaoInstitucional::display_user_avaliacao_list(api_get_user_id());
