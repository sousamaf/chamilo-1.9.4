...
// Exibir ou não o CPF
if(api_get_user_status() == STUDENT || api_get_user_status() == ANONYMOUS)
{
	$show_official_code = false;
}
else 
{
	$show_official_code = true;
}
			...
			case 'export' :
				...
				if($show_official_code == true)
				{
	                ...
	                if (api_get_setting('show_email_addresses') == 'true') {
	                    $select_email_condition = ' user.email, ';
	                    if ($sort_by_first_name) {
	                        $a_users[0] = array('id', get_lang('FirstName'), get_lang('LastName'), get_lang('Username'), get_lang('Email'), get_lang('Active'));
	                    } else {
	                        $a_users[0] = array('id', get_lang('LastName'), get_lang('FirstName'), get_lang('Username'), get_lang('Email'), get_lang('Active'));
	                    }
	                } else {
	                    if ($sort_by_first_name) {
	                        $a_users[0] = array('id', get_lang('FirstName'), get_lang('LastName'), get_lang('Username'), get_lang('Active'));
	                    } else {
	                        $a_users[0] = array('id', get_lang('LastName'), get_lang('FirstName'), get_lang('Username'), get_lang('Active'));
	                    }
	                }
                ...
                if ($_GET['type'] == 'pdf') {
                	if($show_official_code == true)
					{
	                    ...
	                    if ($is_western_name_order) {                        
	                        $a_users[0] = array('#', get_lang('UserPicture'), get_lang('FirstName').', '.get_lang('LastName'));
	                    } else {
	                        $a_users[0] = array('#', get_lang('UserPicture'), get_lang('LastName').', '.get_lang('FirstName'));
	                    }						
					}
                }
                ...
                if (api_get_session_id()) {	
					...
					if($show_official_code == true)
					{
                    	...
                    	$sql_query = "SELECT DISTINCT user.user_id, ".($is_western_name_order ? "user.firstname, user.lastname" : "user.lastname, user.firstname").",  user.username, $select_email_condition phone, active $legal
                                  FROM $table_session_course_user as session_course_user, $table_users as user ";						
					}
                    if ($_configuration['multiple_access_urls']) {
